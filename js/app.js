(function (app) {
	...
	function personalPageSetup() {
		...
		document
			.getElementById("add-deck-cancel")
	...
	function alterDeckButtonGroupSetup() {
		if (getUserId() == app.deck?.ownerId) {
			cardSearchSetup();
			document.querySelector("button.delete-deck").onclick =
				deleteDeckAsync;
			editDeckFormSetup();

			document
				.querySelector(".deck-alter-group")
	...
	function removeCardQuantity(dcid) {
		...
		if (card.quantity <= 0) {
			...
			for (const el of document.querySelector(".cards-container")
				.children) {
	...
	async function deleteDeckAsync() {
		...
		if (!response?.success) {
			document
				...
				.append(errorDiv(response?.errors));
			return;
		} else {
			location.href = `../decks/personal.html`;
		}
